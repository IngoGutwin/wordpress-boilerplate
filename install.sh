...
set_wp_permissions() {
    ...
    sudo find "$wp_path" -type d ! -path "*/node_modules/*" -exec chmod 775 {} \;

    echo "chmod to 664 (files, außer node_modules) ..."
    sudo find "$wp_path" -type f ! -path "*/node_modules/*" -exec chmod 664 {} \;
